"""
This module runs tests on production speed sensors
"""
import time
import random
...
def test_over(mqtt, machine_id, test_number, **kwargs):
    """ Test production speed too high """

    base_payload = test(mqtt, machine_id, test_number,
                        kwargs["delay"], random.randint(61, 100))
    ...
    expected_values = {
        ...
        "value": base_payload["values"]["productionSpeed"],
...
def test(mqtt, machine_id, test_number, delay, value):
    ...
    test_msg = f'Test #{test_number}: starting production speed test on {machine_id}'
    print_color(test_msg, TerminalColor.OKBLUE)

    machine_topic = f"machine/{machine_id}"

    base_payload = get_payload_with_term(
        mqtt, machine_topic, "productionSpeed", test_number, 4)

    if base_payload is None:
        return None

    # edge case
    if base_payload['values']['productionSpeed'] == 'null':
        base_payload['values']['productionSpeed'] = 0

    base_payload['values']['productionSpeed'] += value
...
def main():
    """ Launch over and under production speed test """
