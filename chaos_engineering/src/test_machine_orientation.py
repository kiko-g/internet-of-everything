"""
This module runs tests on machine orientation sensors
"""
import time
import random
...
def test_over(mqtt, machine_id, test_number, **kwargs):
    """ Test orientation too high """

    base_payload = test(mqtt, machine_id, test_number,
                        kwargs["delay"], random.randint(360, 500))
    ...
    expected_values = {
        ...
        "value": base_payload["values"]["orientation1"],
...
def test_under(mqtt, machine_id, test_number, **kwargs):
    """ Test orientation too low """
    base_payload = test(mqtt, machine_id, test_number,
                        kwargs["delay"], -random.randint(0, 70))
    if base_payload is None:
        return -1
...
def test(mqtt, machine_id, test_number, delay, value):
    ...
    test_msg = f'Test #{test_number}: starting orientation test on {machine_id}'
    print_color(test_msg, TerminalColor.OKBLUE)

    machine_topic = f"machine/{machine_id}"

    base_payload = get_payload_with_term(
        mqtt, machine_topic, "orientation1", test_number, 4)

    if base_payload is None:
        return None

    # edge case
    if base_payload['values']['orientation1'] == 'null':
        base_payload['values']['orientation1'] = 0

    base_payload['values']['orientation1'] += value
...
def main():
    """ Launch over and under orientation test """
