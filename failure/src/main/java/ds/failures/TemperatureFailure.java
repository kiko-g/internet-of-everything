...
public class TemperatureFailure extends MachineFailure {
    ...
    @Override
    public String checkMachine(MachineState currentState) {
        ...
        if(currentTemp > tempState.getMaxTemp()) {
            ...
            return this.takeAction(currentState.getId());
        }
        
        return this.futureBehaviour(currentState);
    }
    ...
    public String futureBehaviour(MachineState currentState) {
        ...
        if (close < 10.0) {
            if (numConsecutive > FUTURE_BEHAVIOUR) {
                ...
                return getResponseMessage(currentState.getId(), "Critical", NOERROR_NUM, "Temperature Sensor");
            } else {
                //System.out.println("\n Temperature close to the limit");
                return getResponseMessage(currentState.getId(), "Warning", NOERROR_NUM, "Temperature Sensor");
            }
        ...
        return getResponseMessage(currentState.getId(), "Ok", NOERROR_NUM, "Temperature Sensor");
    }

    @Override
    public String takeAction(String machineID) {
        // System.out.println("MachineID :: " + machineID + " :: Cooling up the Machine");
        return getResponseMessage(machineID, "Failed", ERROR_NUM, "Temperature Sensor");
    }
}
